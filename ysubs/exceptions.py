...
class SignerInvalid(SignatureError):
    def __init__(self, signer: str):
        super().__init__(
            f"The X-Signer header you provided ({signer}) is not a valid address."
        )
...
class BadInput(ValueError):
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(
            *[str(arg) if isinstance(arg, Exception) else arg for arg in args], **kwargs
        )
...
class TooManyRequests(Exception):
    def __init__(self, time_til_next_request: float):
        super().__init__(
            f"You can make your next request in {round(time_til_next_request, 2)} seconds."
        )
