...
def _clear_stale_for(address: EthAddress, t: Optional[float] = None) -> None:
    select(r for r in UserRequest if r.user.address == address and (t or time()) - r.timestamp >= ONE_DAY).delete(bulk=True)
...
def _time_til_next(subscription: "Subscription", limiter: Literal["minute", "day"]) -> float:
    ...
    if limiter == "minute":
        if _count_this_minute(subscription.user) < subscription.plan.requests_per_minute:
            return 0
        next = ONE_MINUTE - (t - select(r.timestamp for r in UserRequest if r.user.address == subscription.user and t - r.timestamp < ONE_MINUTE).min())
    elif limiter == "day":
        _clear_stale_for(subscription.user)
        next = ONE_DAY - (t - select(r.timestamp for r in UserRequest if r.user.address == subscription.user).min())
    else:
...
def _count_this_minute(address: EthAddress) -> int:
    return select(r for r in UserRequest if r.user.address == address and time() - r.timestamp < ONE_MINUTE).count()
...
class UserRequest(db.Entity):
    ...
    async def next(cls, subscription: "Subscription") -> int:
        next = min(await asyncio.gather(*[UserRequest._time_til_next(subscription, period) for period in ["minute", "day"]]))
        return next if next > 0 else 0
    ...
    async def _time_til_next(cls, subscription: "Subscription", limiter: Literal["minute", "day"]) -> float:
        return await asyncio.get_event_loop().run_in_executor(None, _time_til_next, subscription, limiter)
